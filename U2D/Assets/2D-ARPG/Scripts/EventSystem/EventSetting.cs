...
public class EventSetting : MonoBehaviour {
	...
	public void Activate(){
		...
		if(type == ParametersType.ChangeScene){
			...
			if(boolVar){
				mainPlayer.GetComponent<Status>().spawnPointName = stringVar2;
			}
		...
		if(type == ParametersType.LookAtTarget){
			...
			if(boolVar){
				...
				if(boolVar2){
					...
					Vector3 delta = lookPos - t.position; 
		...
		if(type == ParametersType.SetParent){
			if(boolVar && GlobalStatus.mainPlayer){
				target2 = GlobalStatus.mainPlayer.transform;
			}
			if(target && target2){
		...
		if(type == ParametersType.AddCash){
			if(GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer.GetComponent<Inventory>().cash += intVar;
			}
			NextEvent();
		}
		if(type == ParametersType.AddSkillPoint){
			if(GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer.GetComponent<Status>().skillPoint += intVar;
			}
			NextEvent();
		}
		if(type == ParametersType.AddStatusPoint){
			if(GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer.GetComponent<Status>().statusPoint += intVar;
			}
			NextEvent();
		}
		if(type == ParametersType.AddItem){
			if(GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer.GetComponent<Inventory>().AddItem(intVar , intVar2);
			}
			NextEvent();
		}
		if(type == ParametersType.AddEquipment){
			if(GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer.GetComponent<Inventory>().AddEquipment(intVar);
			}
			NextEvent();
		}
		if(type == ParametersType.AddQuest){
			if(GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer.GetComponent<QuestStat>().AddQuest(intVar);
			}
			NextEvent();
		}
	}
	...
	Transform SetTarget(){
		if(targetIsPlayer){
			if(!GlobalStatus.mainPlayer){
				GlobalStatus.mainPlayer = GameObject.FindWithTag("Player");
			}
			if(!GlobalStatus.mainPlayer){
				print("No Player in the scene");
				return null;

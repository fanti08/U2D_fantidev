...
public class CustomEventEditor : Editor{
	...
	public override void OnInspectorGUI(){
		...
		if(script.type == ParametersType.LookAtTarget){
			...
			script.boolVar2 = EditorGUILayout.Toggle("Lock Z Angle", script.boolVar2);
		}
		...
		if(script.type == ParametersType.ConditionCheck){
			...
			script.ev1 = EditorGUILayout.ObjectField("If Pass Event", script.ev1 , typeof(EventActivator), true) as EventActivator;
			script.ev2 = EditorGUILayout.ObjectField("If Fail Event", script.ev2 , typeof(EventActivator), true) as EventActivator;
